...
t("Users can only update their own subdomains", (t) => {
    if (requiredEnvVars.every((v) => process.env[v])) {
        ...
        changedJSONFiles.forEach((file) => {
            ...
            if (data.owner.username === "dicoding-app") {
                t.true(
                    admins.includes(prAuthorId),
                    `${subdomain}: ${prAuthor} is not authorized to update ${subdomain}.capstonedicoding.tech`
                );
            } else {
                t.true(
                    data.owner.username.toLowerCase() === prAuthor || trusted.includes(prAuthorId),
                    `${file}: ${prAuthor} is not authorized to update ${subdomain}.capstonedicoding.tech`
                );
            }
        });
        ...
        deletedJSONFiles.forEach((file) => {
            ...
            if (data.owner.username === "dicoding-app") {
                t.true(
                    admins.includes(prAuthorId),
                    `${subdomain}: ${prAuthor} is not authorized to delete ${subdomain}.capstonedicoding.tech`
                );
            } else {
                t.true(
                    data.owner.username.toLowerCase() === prAuthor || trusted.includes(prAuthorId),
                    `${file}: ${prAuthor} is not authorized to delete ${subdomain}.capstonedicoding.tech`
                );
            }
        });
    }

...
2. Every JSON file must follow the exact filename rules, JSON schema, and validation rules.
...
- "Filenames cannot have uppercase characters. See docs for more information."
...
Every file must be valid JSON and follow this exact structure:
...
- TXT verification records must be paired correctly.
...
- "File does not follow proper schema. See docs for more information."
- "This should be `records` not `record`."
- "Cannot use CNAME in conjunction with other records and vice versa. See docs for more information."
...
- "Invalid username, username does not match the GitHub account that is opening the pull request. See docs for more information (trusted maintainers exempt — see util/trusted.json)"
- "Invalid CNAME. CNAME cannot contain protocols, file paths, URL parameters, or anything combination of those. CNAME records must be lowercased. See docs for more information. Reopen PR when this is fixed."
...
- Always link back to the documentation.

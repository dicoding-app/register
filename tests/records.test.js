...
function validateRecordValues(t, data, file) {
    ...
    Object.entries(data.records).forEach(([key, value]) => {
        ...
        if (["CNAME", "URL"].includes(key)) {
            ...
            if (key === "CNAME") {
                ...
                t.true(value !== `${subdomain}.capstonedicoding.tech`, `${file}: ${key} cannot point to itself`);
                t.true(value !== "capstonedicoding.tech", `${file}: ${key} cannot point to capstonedicoding.tech`);
                ...
                t.false(urlHost === `${subdomain}.capstonedicoding.tech`, `${file}: ${key} cannot point to itself`);
            }
        }
    ...
    if (data.redirect_config) {
        ...
        customPaths.forEach((customPath, idx) => {
            ...
            t.false(urlHost === `${subdomain}.capstonedicoding.tech`, `${urlMessage} cannot point to itself at index ${idx}`);
        });
    }
}

...
for (const subdomain of internal) {
    const entry = {
        domain: `${subdomain}.capstonedicoding.tech`,
        subdomain: subdomain,
        owner: { username: "capstones-tech" },
        records: { CNAME: "internal.capstonedicoding.tech" },
        internal: true
    };
    v2.push(entry);
...
for (const subdomain of reserved) {
    const entry = {
        ...
        subdomain: subdomain,
        owner: { username: "capstones-tech" },
        records: { URL: "https://capstonedicoding.tech/reserved" },
        reserved: true
    };
    v2.push(entry);
    writeDomainFile(entry);
...
fs.readdir(directoryPath, function (err, files) {
    ...
    files.forEach(function (file) {
        ...
        fs.readFile(filePath, "utf8", (err, data) => {
            ...
            item.domain = `${name}.capstonedicoding.tech`;
            item.subdomain = name;
